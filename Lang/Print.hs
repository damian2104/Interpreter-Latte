...
instance Print Program where
  prt i e = case e of
    Program alldefs -> prPrec i 0 (concatD [prt 0 alldefs])

instance Print AllDef where
  prt i e = case e of
    FunDef topdef -> prPrec i 0 (concatD [prt 0 topdef])
    VarDef topvar -> prPrec i 0 (concatD [prt 0 topvar])
  prtList _ [x] = (concatD [prt 0 x])
  prtList _ (x:xs) = (concatD [prt 0 x, prt 0 xs])
instance Print TopVar where
  prt i e = case e of
    VarDecl type_ items -> prPrec i 0 (concatD [prt 0 type_, prt 0 items])
...
instance Print Stmt where
  prt i e = case e of
    ...
    For id expr1 expr2 stmt -> prPrec i 0 (concatD [doc (showString "for"), prt 0 id, doc (showString "="), prt 1 expr1, doc (showString "to"), prt 1 expr2, prt 0 stmt])
    SExp expr -> prPrec i 0 (concatD [prt 0 expr, doc (showString ";")])
    Prt expr -> prPrec i 0 (concatD [doc (showString "print("), prt 4 expr, doc (showString ")")])
    FunDecl topdef -> prPrec i 0 (concatD [prt 0 topdef])
...
instance Print Expr where
  prt i e = case e of
    ...
    ELitInt n -> prPrec i 4 (concatD [prt 0 n])
    ELitTrue -> prPrec i 4 (concatD [doc (showString "true")])
    ELitFalse -> prPrec i 4 (concatD [doc (showString "false")])
    EApp id exprs -> prPrec i 4 (concatD [prt 0 id, doc (showString "("), prt 0 exprs, doc (showString ")")])
    EString str -> prPrec i 4 (concatD [prt 0 str])
    Neg expr -> prPrec i 3 (concatD [doc (showString "-"), prt 4 expr])
    Not expr -> prPrec i 3 (concatD [doc (showString "!"), prt 4 expr])
    EMul expr1 mulop expr2 -> prPrec i 2 (concatD [prt 2 expr1, prt 0 mulop, prt 3 expr2])
    EAdd expr1 addop expr2 -> prPrec i 1 (concatD [prt 1 expr1, prt 0 addop, prt 2 expr2])
    ERel expr1 relop expr2 -> prPrec i 0 (concatD [prt 0 expr1, prt 0 relop, prt 1 expr2])
  prtList _ [] = (concatD [])
  prtList _ [x] = (concatD [prt 0 x])
  prtList _ (x:xs) = (concatD [prt 0 x, doc (showString ","), prt 0 xs])
